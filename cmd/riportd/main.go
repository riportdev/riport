...
import (
	...
	"github.com/riportdev/riport/cmd/rportd/servicemanagement"
	...
	chserver "github.com/riportdev/riport/server"
	"github.com/riportdev/riport/server/api/message"
	auditlog "github.com/riportdev/riport/server/auditlog/config"
...
const (
	...
	DefaultPairingURL                       = "https://pairing.riport.io"
)
...
var serverHelp = `
  ...
  Examples:
    ...
    ./rportd -c /etc/riport/rportd.conf
  ...
  Options:
    ...
    This is where the riport clients connect to.
    ...
    This is for authentication of the riport tunnel clients.
    ...
    Ideally this directory is the homedir of the riport user and has been created along with the user.
    Example: useradd -r -d /var/lib/rportd -m -s /bin/false -U -c "System user for riport client and server" riport
    ...
    --max-request-bytes-client, An optional arg to define a limit for data that can be sent by riport clients.
    By default is set to 524288(512Kb).

    --allow-root, An optional arg to allow running rportd as root. There is no technical requirement to run the riport
    ...
    --service-user, An optional arg specifying user to run rportd service under. Only on linux. Defaults to riport.
...
func init() {
	...
	if runtime.GOOS != "windows" {
		svcUser = lFlags.String("service-user", "riport", "")
	}
	...
	viperCfg.SetDefault("notifications.notification_script_dir", "/usr/local/lib/riport/notification_scripts")
}
...
func runMain(*cobra.Command, []string) {
	...
	if writeCPUProfile {
		...
		cpuprof, err := os.Create("/var/lib/riport/cpu.rportd.prof")
		if err != nil {
			log.Fatal(err)
		}
...
func WriteMemoryProfile(l *logger.Logger) {
	...
	memf, err := os.Create("/var/lib/riport/mem.rportd.prof")
	if err != nil {

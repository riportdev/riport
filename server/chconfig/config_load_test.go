...
func TestLoadingExampleConf(t *testing.T) {
	...
	assert.Equal(t, "/var/lib/riport", cfg.Server.DataDir)
}

const (
	sampleCfg = `
[plus-plugin]
	plugin_path = "/usr/local/lib/riport/riport-plus.so"
[plus-license]
  id = "<your-license-id>"
...
func TestLoadingPlusConf(t *testing.T) {
	...
	assert.Equal(t, "/usr/local/lib/riport/riport-plus.so", cfg.PlusConfig.PluginConfig.PluginPath)

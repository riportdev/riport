...
// GetAll returns riport clients auth credentials from a given file.
func (c *FileProvider) getAll() ([]*ClientAuth, error) {
	idPswdPairs, err := c.load()
	if err != nil {
		return nil, fmt.Errorf("failed to decode riport clients auth file: %v", err)
	}
...
func (c *FileProvider) Add(clientAuth *ClientAuth) (bool, error) {
	idPswdPairs, err := c.load()
	if err != nil {
		return false, fmt.Errorf("failed to decode riport clients auth file: %v", err)
	}
	...
	if err := c.save(idPswdPairs); err != nil {
		return false, fmt.Errorf("failed to encode riport clients auth file: %v", err)
	}
...
func (c *FileProvider) Delete(id string) error {
	idPswdPairs, err := c.load()
	if err != nil {
		return fmt.Errorf("failed to decode riport clients auth file: %v", err)
	}
	...
	if err := c.save(idPswdPairs); err != nil {
		return fmt.Errorf("failed to encode riport clients auth file: %v", err)
	}
...
func (c *FileProvider) load() (map[string]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to read riport clients auth file %q: %s", c.fileName, err)
	}
...
func (c *FileProvider) save(idPswdPairs map[string]string) error {
	...
	if err != nil {
		return fmt.Errorf("failed to open riport clients auth file: %v", err)
	}
	defer file.Close()
	...
	if err := encoder.Encode(idPswdPairs); err != nil {
		return fmt.Errorf("failed to write riport clients auth: %v", err)
	}

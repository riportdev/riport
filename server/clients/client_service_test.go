...
import (
	...
	apiErrors "github.com/riportdev/riport/server/api/errors"
...
// go test -count=1 -race -v github.com/riportdev/riport/server/clients -run TestStartClientConcurrency
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			internalTunnelProxyConfig := &clienttunnel.InternalTunnelProxyConfig{
				Enabled:  true,
				CertFile: "../../../testdata/certs/riport.test.crt",
				KeyFile:  "../../../testdata/certs/riport.test.key",
			}
			...
			requestedRemote.TunnelURL = "https://12345678.tunnels.riport.test"
			...
			assert.Equal(t, "tunnels.riport.test", mockCaddyAPI.NewRouteRequest.DownstreamProxyBaseDomain)

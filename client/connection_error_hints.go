...
func ConnectionErrorHints(server string, logger *logger.Logger, err error) error {
	...
	case "Access violation":
		return fmt.Errorf("%s - Check your proxy allows the CONNECT method to the riport server port", err)
	case "Proxy Authentication Required":
		return fmt.Errorf("%s - Check the proxy username and password in riport.conf or add if missing", err)
